...
export interface ProductJson
  extends Omit<RawProduct, "text_embeddings" | "images"> {
  _id: string;
  slug: string;
...
export const ProductSearchQuery =
  mongoose?.models?.ProductSearchQuery ||
  mongoose.model("ProductSearchQuery", ProductSearchQuerySchema);
...
ProductSchema.methods.unlike = async function (userId: string) {
  const existingLike = await ProductLike.findOneAndDelete({
    userId,
    productId: this.id,
  });
