...
export default class ProductParser {
    ...
    if (jsonLdScript) {
      try {
        ...
        const product: ProductDetails = {
          ...
          price: productData.offers
            ? Number.parseFloat(productData.offers.price)
            : undefined,
          currency: productData.offers
  ...
  private parseHM($: cheerio.CheerioAPI): ProductDetails | null {
    ...
    const priceText = $('meta[property="product:price:amount"]').attr(
      "content",
    );
  ...
  private parseNykaa($: cheerio.CheerioAPI): ProductDetails | null {
    ...
    const priceText = $("span.product-price")
      .text()
      .trim()
      .replace(/[^\d\.]/g, "");
