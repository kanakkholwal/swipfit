...
async function importToPineCone() {
    ...
    for await (const product of products) {
        ...
        try {
            ...
            const embedding = await pinecone.inference.embed(model,
            ...
            vectors.push({
                ...
                values:Array.from(embedding.data[0].vectorType === 'dense' ? embedding.data[0].values : []),
                metadata: {
                    title: product.title,
            ...
            await pineconeIndex.namespace(`products-${product.genderGroup}`).upsert(vectors);
        } catch (error) {
